...
# ═══ FILTRE DE REGIÓ ═══
# Nomes es guarden estacions dins d'aquest requadre lat/lon.
# Canvia aquests valors quan vulguis ampliar/reduir la zona.
...
def transformar(datos_crudos):
    ...
    for reg in datos_crudos:
        ...
        # Filtro de región: descartamos todo lo que caiga fuera del
        # cuadro definido en REGIO, antes de gastar tiempo en lo demás.
        if not (REGIO["lat_min"] <= lat <= REGIO["lat_max"] and
        ...
        por_estacion[idema] = {
            ...
            "hr": redondear(reg.get("hr"), 0),  # humedad relativa (%)
            "vv": redondear(reg.get("vv"), 1),  # velocidad media del viento (m/s)
            "dv": redondear(reg.get("dv"), 0),  # dirección media del viento (grados)
            "prec": redondear(reg.get("prec"), 1),  # precipitación acumulada última hora (mm)
            "fint": fint,
        }
    ...
    print(f"[INFO] Región activa: lat [{REGIO['lat_min']}, {REGIO['lat_max']}] · "
          f"lon [{REGIO['lon_min']}, {REGIO['lon_max']}]")
